...
1. Shared CLI behavior has no stable normative home, so post-self-hosting Stage 2 parity work depends on reading Stage 1 code and old
...
3. Explicitly reserve one section for concrete stage-specific differences that the shared contract must call out rather
   than hide, including:
    ...
    - launcher/environment defaulting expectations (`L0_HOME`, `L0_SYSTEM`, `L0_RUNTIME_INCLUDE`, `L0_CFLAGS`) where
      they affect user-visible CLI behavior
4. Keep stage-specific text out of the shared spec unless the difference is explicit and concrete.
5. Update `docs/specs/compiler/stage1-contract.md` so it becomes a Stage 1 contract plus navigation index that points
...
8. Do not create a separate Stage 2 contract in this change; the goal is one shared CLI spec plus the existing
