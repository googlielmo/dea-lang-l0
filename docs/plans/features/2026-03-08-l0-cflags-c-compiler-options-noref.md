...
1. New environment variable contract:
   `L0_CFLAGS` provides extra C compiler flags/options for `--build` and `--run` paths.
2. Precedence/ordering contract:
   effective option vector is `split(L0_CFLAGS) + split(--c-options)`.
