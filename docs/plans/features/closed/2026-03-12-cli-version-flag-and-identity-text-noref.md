...
top of `--help`. The returned string should come from a function rather than being inlined so a later change can
replace it with a semantic version, commit hash, build id, or similar metadata without reshaping the CLI code.
