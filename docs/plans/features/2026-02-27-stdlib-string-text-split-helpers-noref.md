...
2. `split_s` rejects `sep == ""` with assertion message:
   `split_s: separator must be non-empty`.
...
6. `replace_s` rejects `old == ""` with assertion message:
   `replace_s: old pattern must be non-empty`.
