...
`ParseError`
from the external API. The parser now implements synchronization loops at statement and declaration boundaries, allowing
it to continue analyzing source files after encountering malformed constructs and providing comprehensive multi-error
reports.
...
2. **`l0_parser.py`**:
    ...
    - Introduced `_error` (log and continue) and `_error_bail` (log and raise internal synchronization exception)
      helper methods.
...
4. **`l0_diagnostics.py`**:
    - Broke a circular dependency with `l0_lexer.py` by using `TYPE_CHECKING` and forward references for the `Token`
      type in diagnostic utility functions.
5. **`l0c.py`**:
...
- Updated roughly 30 tests across multiple modules to check the `diagnostics` list instead of expecting
  `LexerError` or `ParseError` exceptions.
