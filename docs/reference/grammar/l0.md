...
* If header initializers may short-circuit via `?`, cleanup must not reference
  non-nullable header variables that may be uninitialized on those failure paths.
  Use nullable header types for variables that cleanup needs to inspect.
