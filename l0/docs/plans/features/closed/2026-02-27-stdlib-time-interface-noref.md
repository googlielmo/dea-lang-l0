...
01. `struct WallTime { sec: int; nsec: int; }`
02. `struct MonotonicTime { sec: int; nsec: int; }`
03. `struct Duration { sec: int; nsec: int; }`
04. `struct DateTime { year, month, day, hour, minute, second, nanosecond, weekday, yearday, utc_offset_sec, is_dst }`
05. `wall_now() -> WallTime?`
06. `monotonic_supported() -> bool`
07. `monotonic_now() -> MonotonicTime?`
08. `monotonic_diff(start: MonotonicTime, end: MonotonicTime) -> Duration?`
09. `wall_to_utc_datetime(t: WallTime) -> DateTime?`
