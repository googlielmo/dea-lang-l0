...
The current Stage 2 `--version` path prints only the static identity string: `Dea language / L0 compiler (Stage 2)`.
That is sufficient for source-tree parity, but it does not answer the operational questions an installed compiler should
answer: which build produced it, when it was built, which commit it came from, which host built it, and which host
compiler banner identifies the actual native toolchain.
...
01. Add a dedicated Stage 2 module, `build_info.l0`, that owns the values used by `--version`. The checked-in
    source-tree copy should provide default or unknown values only, so normal source-tree and raw self-hosted builds
    remain on the static fallback path.
02. Keep `--help` and verbose startup identity unchanged. Only the `--version` path should consult `build_info.l0`. When
    the embedded fields are absent or unknown, `--version` should fall back to the current single-line Stage 2 identity
    output.
03. Add one shared Python helper in the build/install tooling that collects a build-provenance snapshot:
...
04. Host platform text should use `uname -s`, `uname -r`, and `uname -m` on Unix-like systems, joined as
    `<kernel-name> <kernel-release> <machine>`. On non-Unix systems it should use a portable fallback with the same
    three-field shape.
05. Repository cleanliness should come from git state at build time and should be expressed only in the `commit:` line:
    - clean tree: `<fullsha>`
    - dirty tree: `<fullsha>+dirty`
06. Resolve the host C compiler once in Python using the same precedence already used by the compiler drivers:
    - `L0_CC`
    - `tcc`, `gcc`, `clang`, `cc`
    - `CC` Then export that exact value back into the subprocess environment as `L0_CC` so the captured compiler banner
      and the actual build compiler agree.
07. For `install-dev-stage2`, `scripts/build_stage2_l0c.py` should:
...
08. For `install`, `scripts/gen_dist_tools.py install-prefix` should:
...
09. Do not use the overlay in raw Stage 2 `--build -o ...` invocations. In particular, do not use it in the
...
15. The GitHub Actions auto-derived build ID format should be:
    - `gha-<run_id>.<run_attempt>-<job>-<runner_os>-<runner_arch>` using:
    - `GITHUB_RUN_ID`
    - `GITHUB_RUN_ATTEMPT`
    - `GITHUB_JOB`
    - `RUNNER_OS`
...
1. Repo-local and install-prefix Stage 2 artifacts print the multiline embedded build-provenance report on both wrapper
   and `.native` `--version`.
...
5. The strict triple-bootstrap regression still compares byte-stable compiler 2 / compiler 3 `.c` and `.native` outputs,
   with no generated build metadata leaking into those artifacts.
