...
1. It was missing large parts of the normal `-v` and `-vvv` stderr contract: effective roots, compilation-unit progress,
   pass summaries, backend/codegen milestones, build-driver compiler details, and `--run` execution tracing.
...
The fix restores the Stage 1 wording, ordering, and log-level split for equivalent Stage 2 code paths while keeping the
Stage 2 identity text unchanged.
...
2. Reworked driver-side lex/parse sequencing so Stage 2 can emit: `Resolved ...`, `Lexing ...`, `Lexed N token(s) ...`,
   `Parsing ...`, and `Parsed module ...` in the same order as Stage 1.
