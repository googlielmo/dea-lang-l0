...
As a result, a directly built Stage 2 binary could not resolve `std.io` and other stdlib modules when launched with only
`L0_HOME` configured.

## Fix Implemented

1. Update Stage 2 source-path defaulting to match Stage 1 precedence: explicit sys-root suppresses defaults, then
   `L0_SYSTEM`, then `L0_HOME/shared/l0/stdlib`.
2. Keep `L0_SYSTEM` authoritative when set.
3. Add a focused Stage 2 unit test for: no env values, `L0_SYSTEM` precedence, `L0_HOME` fallback, and explicit sys-root
   suppression.
