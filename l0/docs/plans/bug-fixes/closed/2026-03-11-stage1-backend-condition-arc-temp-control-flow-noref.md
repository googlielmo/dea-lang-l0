...
especially ARC temp declarations. The concrete repro was in `compiler/stage2_l0/src/parser/shared.l0`:

```l0
while (look < n && tok_is(tv_get(self.tokens, look), ord(TT_DOT))) {
```

When Stage 1 built the Stage 2 compiler, the generated C materialized `tv_get(self.tokens, look)` once before the loop
and reused that stale temp across iterations. That broke parser lookahead and more generally meant control-flow
