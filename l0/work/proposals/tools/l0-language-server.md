...
1. JSON support. LSP is JSON-RPC. Without a parser and serializer, the server cannot decode requests or encode
   responses.
2. Byte-precise stream I/O. LSP framing uses headers plus exact payload byte counts. Current `read_line()` and
   `read_char()` are insufficient for efficient and correct framed message transport.
3. Path and URI utilities. The server needs path normalization, joining, relative/absolute handling, basename/dirname,
   and `file://` URI conversion.
4. Better filesystem APIs. The runtime can already answer basic existence/delete queries, but a usable tooling layer
   still needs wrappers plus directory enumeration and metadata access.
