...
definite runtime problems (leaks, malformed critical events, panic actions, impossible balances). Keep it independent of
capture so it works on archived logs too.
...
2. Memory lifecycle checks ([l0][mem])
   ...
   3. Compatibility path: if a tracked new_alloc ptr is finalized by op=free action=call, decrement balance and emit a
      warning (preferred finalization remains drop/free).
...
01. Unit/CLI tests in compiler/stage1_py/tests/cli/test_stage2_trace_log_checker.py (so they run in existing pytest
    path).
02. Test: clean balanced log returns exit 0.
03. Test: new_alloc without matching drop free returns exit 1 and reports leak pointer.
04. Test: alloc_string without matching terminal free_string action=free returns exit 1.
05. Test: free_string action=decrement-only alone does not close lifecycle and does not falsely pass a leaked
    allocation.
06. Test: drop free without prior new_alloc returns exit 1 (negative balance).
07. Test: ARC panic action triggers exit 1.
08. Test: heap free release with rc_after != 0 triggers exit 1.
09. Test: malformed required fields (ptr missing on critical events) triggers exit 1.
10. Smoke test: run checker on a real parser trace artifact from run_parser_trace.sh and assert the command executes and
    emits a structured report (stats/op_counts/errors/warnings), with exit code interpreted as analysis result.
