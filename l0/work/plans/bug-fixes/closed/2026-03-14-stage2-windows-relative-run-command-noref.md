...
Windows Stage 2 `--run` still fails when the driver falls back to the current directory for temp artifacts. The temp
executable path becomes `./l0c-stage2-run-...exe`, and `system()` routes that through `cmd.exe`, which treats the
