...
The final user-visible output is intentionally Stage 2-specific where identity text is involved: `--help` now prints
`Dea language / L0 compiler (Stage 2)`, while no-argument failures keep the existing
...
2. The built Stage 2 launcher prints full help with the exact heading `Dea language / L0 compiler (Stage 2)` on
   `--help`.
