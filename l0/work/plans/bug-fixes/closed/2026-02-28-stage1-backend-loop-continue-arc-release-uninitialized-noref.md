...
The generated C still emits `rt_string_release(...)` for that local at loop-iteration end, even on the early `continue`
path where the local was never initialized.
...
6. Keep the Stage 2 workaround in place until the Stage 1 fix is validated, then decide separately whether to simplify
   it.
...
The backend previously used one loop-cleanup path for both `break` and `continue`. That was wrong for `for` loops
because:
...
7. no generated C path releases an ARC local before initialization or skips cleanup for already-acquired loop-body ARC
   locals on `continue`, `break`, or `return`.
...
2. The defect is broader than `name_resolver.l0`; any similar loop shape with a post-`continue` ARC local may be
   affected.
