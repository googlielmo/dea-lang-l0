...
The pending stdlib boundary cleanup in `work/plans/refactors/closed/2026-03-13-stdlib-fs-io-boundary-cleanup-noref.md`
is part of the landing criteria for this feature. The safe byte-stdio surface and the `std.fs` / `std.io` path-helper
split must ship together.
...
3. New shared fixed-size buffer type: `std.array::ByteArray { storage: ArrayBase*; }`
4. New `std.array` API:
...
4. `write_*_some` return:
   ...
   - `null` on I/O error.
5. `write_*_all` loop on `*_some` until all requested bytes are written, and return `null` on I/O error or zero-progress
   write.
...
6. Land the pending `std.fs` / `std.io` path-helper refactor in
   `work/plans/refactors/closed/2026-03-13-stdlib-fs-io-boundary-cleanup-noref.md` in the same change set.
...
2. The `std.fs` / `std.io` boundary cleanup in
   `work/plans/refactors/closed/2026-03-13-stdlib-fs-io-boundary-cleanup-noref.md` is implemented in the same landing.
